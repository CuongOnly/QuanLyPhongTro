...
namespace HotelManagementSystem
{
    public partial class admin_addUser : UserControl
    {
                ...
                using(SqlConnection connect = new SqlConnection(conn))
                {
                    ...
                    using (SqlCommand checkU = new SqlCommand(checkUsern, connect))
                    {
                        ...
                        if (table.Rows.Count > 0)
                        {
                            ...
                            MessageBox.Show($"{tempUsern} đã tồn tại", "Thông báo lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        } else if (addUser_password.Text.Length < 8)
                        {
                            MessageBox.Show("Mật khẩu trên 8 kí tự", "Thông báo lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void addUser_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn Cập nhật ID: "+ getID + "?", "Xác nhận thoát", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        private void addUser_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa ID: " + getID + "?", "Xác nhận thoát", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
